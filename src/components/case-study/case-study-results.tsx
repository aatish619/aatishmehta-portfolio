...
export function CaseStudyResults({ project }: { project: Project }) {
  ...
  return (
    <div className="border-b border-border/30 py-10 lg:py-14">
      <ScrollReveal className="flex flex-col gap-8">
        <Heading size="h3">Outcomes & Achievements</Heading>

        <StaggerChildren initialDelay={0.2} staggerDelay={0.1} className="grid gap-4 md:grid-cols-3">
        ...
        <ul className="mt-2 grid gap-3 sm:grid-cols-2">
          {project.achievements.map((achievement, i) => (
            <li key={i} className="flex items-start gap-3 rounded-2xl border border-border/50 bg-card/30 p-4">
